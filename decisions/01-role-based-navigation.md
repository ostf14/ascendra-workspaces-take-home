...
- Persona switcher lives on a meta-bar above the product chrome, labeled `Viewing as Engineer / Admin`. The meta-bar treatment makes the switcher read as viewer-perspective control (whose eyes am I looking through), not as product section navigation. Rendered only for users with multi-persona access; single-role users see clean product chrome without the meta layer.
- Meta-bar background is `--surface-tertiary` (subtly darker than the page surface), height 32px, smaller text than the product nav, separated by a 1px `--border-subtle`. It reads as OS-level chrome — glance up, understand the frame, work within the product below.
- The switcher went through two placements before landing on the meta bar. The first iteration used two plain `Link` elements ("Workspaces" / "Admin") — those read as peer tab navigation, implying admin and developer are peer sections of one site. The second iteration promoted them into a segmented control inside the product top-nav — closer, but still framed as a product control. Moving it up to the meta bar and relabeling to `Engineer / Admin` names the actual thing: a viewer-perspective switch. Order is `Engineer` first, `Admin` second — engineer is the base persona everyone has; admin is elevated.
- Segment last-visited URL memory lives in `localStorage` (`ascendra:last-path:engineer` / `ascendra:last-path:admin`), separate from role. An admin scrolling `/admin/utilization` who flips to Engineer and back returns to `/admin/utilization`, not `/admin`.
