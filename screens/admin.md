...
- Chart is wrapped in a definite-height container (`h-[320px]`) on this page. Recharts' `ResponsiveContainer` renders with `height: 100%` and only resolves against a parent whose `height` is a definite value. On the overview surface the sibling grid cell supplies that height via `items-stretch`; on this standalone page there is no sibling, so the wrapper does it directly. Without the wrapper the chart card ends up 316px tall (from `min-h-[280px]` + content flow) but the `ResponsiveContainer` inside sees a `height: 0` percentage resolution and paints nothing.
