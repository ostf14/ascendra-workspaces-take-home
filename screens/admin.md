...
**Sticky panel:** the right column is `position: sticky` with `top: 144px` (meta-bar 32px + top-nav 56px + admin sub-nav 44px + 12px breathing room) and `max-height: calc(100vh - 168px)` with internal overflow-y. As the admin scrolls through a long inventory table, the selected workspace's detail panel stays pinned within the viewport; the panel's internal content scrolls if it exceeds the available height. Filter row and page header are not sticky.
...
- **Distribution chart** (top-left) — a hybrid histogram + action ribbon in a single card. Top ~55% is a vertical histogram with five equal-width bars (0-10 · 10-30 · 30-60 · 60-85 · 85-100), count above each bar tip, x-axis tick labels below. Behind the bars sit three continuous semantic zone bands — Idle (`--status-idle` at 8% over buckets 0-1), Healthy (`--status-running` at 6% over buckets 2-3), Near capacity (`--status-pending` at 8% over bucket 4) — with matching zoned uppercase labels above the plot. Bottom ~40% is three summary rows, one per zone, each tying the count to a next action: Idle carries a mono recoverable-cost figure (`--status-idle`) plus a `Stop idle workspaces →` link to `/admin/workspaces?idleOnly=true`; Healthy shows an em-dash (no action, that's the point); Near capacity carries a `Consider larger templates →` link to `/admin/templates`. Zone counts and idle recoverable dollars are derived client-side by summing the underlying buckets. The earlier iteration rendered five horizontal bars with per-row recoverable annotations — accurate but visually neutral. The vertical histogram makes the shape scannable (tall left tail = idle problem) and the zoned summary attaches money and a next step to each shape.
